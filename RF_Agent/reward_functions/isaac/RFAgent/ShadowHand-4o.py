...
def compute_reward(object_rot: torch.Tensor, goal_rot: torch.Tensor,
                   object_angvel: torch.Tensor, object_pos: torch.Tensor,
                   goal_pos: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    device = object_rot.device

    # Cosine similarity for orientation alignment (normalized dot product)
    dot_product = (object_rot * goal_rot).sum(dim=-1)
    object_rot_magnitude = torch.norm(object_rot, dim=-1)
    goal_rot_magnitude = torch.norm(goal_rot, dim=-1)
    orientation_similarity = dot_product / (object_rot_magnitude * goal_rot_magnitude + 1e-6)
    
    # Exponential transformation of orientation similarity
    orientation_temp = 5.0
    orientation_reward_transformed = torch.exp(orientation_temp * (orientation_similarity - 1.0))
    
    # Penalize angular velocity to encourage stability
    angvel_penalty = torch.norm(object_angvel, dim=-1)
    angvel_temperature = 0.5
    angvel_penalty_transformed = torch.exp(-angvel_temperature * angvel_penalty)

    # Position alignment reward using exponential decay
    position_diff = torch.norm(object_pos - goal_pos, dim=-1)
    position_temperature = 1.0
    position_reward_transformed = torch.exp(-position_temperature * position_diff)

    # Bonus for achieving a tight orientation and position tolerance
    goal_orientation_tolerance = 0.05
    goal_position_tolerance = 0.02
    orientation_goal_achieved = (orientation_similarity > (1 - goal_orientation_tolerance)).float()
    position_goal_achieved = (position_diff < goal_position_tolerance).float()
    goal_bonus = (orientation_goal_achieved * position_goal_achieved) * 1.0

    # Total reward combining all components
    total_reward = orientation_reward_transformed * 0.6 + \
                   position_reward_transformed * 0.3 + \
                   angvel_penalty_transformed * 0.1 + \
                   goal_bonus

    # Return total reward and individual components
    return total_reward, {
        "orientation_reward": orientation_reward_transformed,
        "position_reward": position_reward_transformed,
        "angvel_penalty": angvel_penalty_transformed,
