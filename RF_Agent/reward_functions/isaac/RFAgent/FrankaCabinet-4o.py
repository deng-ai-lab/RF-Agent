...
class FrankaCabinetGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.rigid_body_states, self.hand_handle, self.drawer_handle, self.cabinet_dof_pos)
...
@torch.jit.script
def compute_reward(rigid_body_states: torch.Tensor,
                   hand_handle: int,
                   drawer_handle: int,
    ...
    distance_penalty = -torch.log1p(distance_temp * distance_to_drawer)

    # Reward for opening the drawer with smoother gradient
    drawer_open_progress = cabinet_dof_pos[:, 3]
    normalized_open_progress = torch.clip(drawer_open_progress, 0.0, 1.0)
    drawer_open_reward = torch.log1p(open_progress_temp * normalized_open_progress)

    # Consistent bonus for maintaining drawer in an open state
    open_maintenance_bonus = torch.where(normalized_open_progress > 0.8, 
                                         torch.tensor(0.5, device=normalized_open_progress.device), 
                                         torch.tensor(0.0, device=normalized_open_progress.device))

    # Calculate total reward
    total_reward = distance_penalty + drawer_open_reward + open_maintenance_bonus

    # Bundle the individual components
    reward_components = {
        'distance_penalty': distance_penalty,
        'drawer_open_reward': drawer_open_reward,
        'open_maintenance_bonus': open_maintenance_bonus
    }
