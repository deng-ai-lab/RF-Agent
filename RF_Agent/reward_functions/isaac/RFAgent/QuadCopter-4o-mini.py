...
def compute_reward(root_positions: torch.Tensor, root_linvels: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Define target position for hovering
    target_x = 0.0
    target_y = 0.0
    target_z = 1.0
    
    # Compute the distance to the target position
    dist_x = target_x - root_positions[..., 0]
    dist_y = target_y - root_positions[..., 1]
    dist_z = target_z - root_positions[..., 2]
    distance = torch.sqrt(dist_x ** 2 + dist_y ** 2 + dist_z ** 2)
    
    # Define rewards and bonuses
    hover_bonus_threshold = 0.3  # Lower hover bonus threshold for stronger incentive
    hover_reward = (distance < hover_bonus_threshold).float() * (1.0 - distance / hover_bonus_threshold) * 1.5  # Increase the weight of hover reward
    stability_penalty = 0.5 * torch.exp(-torch.abs(root_positions[..., 2] - target_z))  # Adjust stability reward
    

    distance_penalty = 0.5 * torch.exp(-2 * distance)  # Diminished penalty for distance with increased sensitivity
    
    # Total reward calculation
    total_reward = distance_penalty + hover_reward + stability_penalty
