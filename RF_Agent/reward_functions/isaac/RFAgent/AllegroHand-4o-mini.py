...
def compute_reward(object_rot: torch.Tensor, goal_rot: torch.Tensor, object_angvel: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Compute the angular distance between current and goal orientations
    angle_diff = torch.acos(torch.clamp((object_rot * goal_rot).sum(dim=-1), min=-1.0, max=1.0))
    
    # Define temperatures for reward transformation
    temperature_angle = 0.1
    temperature_goal_bonus = 1.5
    temperature_velocity_penalty = 0.5
    
    # Negative exponential reward for reducing angular difference
    reward_orientation = -torch.exp(-angle_diff / temperature_angle)
    
    # Improvement bonus for reducing the angle difference since the last step
    improvement_bonus = torch.clamp((angle_diff.max() - angle_diff), min=0) / (angle_diff.max() + 1e-6) * 0.5
    
    # Goal achievement bonus for achieving close proximity to the target orientation
    goal_bonus = (angle_diff < 0.03).float() * temperature_goal_bonus
    
    # Penalty for excessive angular velocity to encourage smooth transitions
    angvel_penalty = -torch.exp(-object_angvel.norm(p=2, dim=-1) / temperature_velocity_penalty)

    # Total reward combining orientation, improvement, goal, and penalty
    total_reward = reward_orientation + improvement_bonus + goal_bonus + angvel_penalty
    
    # Dictionary for individual reward components
    reward_components = {
        'orientation_reward': reward_orientation,
        'improvement_bonus': improvement_bonus,
        'goal_bonus': goal_bonus,
        'velocity_penalty': angvel_penalty
    }
