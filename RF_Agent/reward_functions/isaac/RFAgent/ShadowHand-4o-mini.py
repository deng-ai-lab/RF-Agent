...
class ShadowHandGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_pos, self.object_rot, self.goal_pos, self.goal_rot, self.object_angvel)
...
@torch.jit.script
def compute_reward(object_pos: torch.Tensor, object_rot: torch.Tensor, goal_pos: torch.Tensor, goal_rot: torch.Tensor, object_angvel: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    """{The reward function combines positional proximity, rotational alignment, and velocity stabilization into a unified reward structure, emphasizing the importance of achieving the goal through sequential milestones and encouraging gradual completion.}"""
    ...
    orientation_temp: float = 0.5

    # Compute position and orientation distances
    position_distance = torch.norm(object_pos - goal_pos)
    dot_product = torch.sum(object_rot * goal_rot, dim=-1)
    orientation_error = torch.acos(torch.clamp(dot_product, -1.0, 1.0))
    
    # Stability penalty based on angular velocity
    angular_velocity_magnitude = torch.norm(object_angvel, dim=-1)
    stability_penalty = torch.exp(-stability_weight * angular_velocity_magnitude)

    # Rewards based on distances
    position_reward = torch.exp(-position_weight * position_distance)
    orientation_reward = torch.exp(-orientation_temp * orientation_error)
    
    # Goal achievement bonus
    goal_bonus = torch.where(position_distance < goal_tolerance, torch.tensor(1.0, device=object_pos.device), torch.tensor(0.0, device=object_pos.device))

    # Total reward calculation
    total_reward = position_reward + orientation_reward - stability_penalty + goal_bonus
