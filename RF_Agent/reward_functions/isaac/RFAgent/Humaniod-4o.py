...
def compute_reward(root_states: torch.Tensor, targets: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Extract velocity, position, and angular velocity from root_states
    velocity = root_states[:, 7:10]
    torso_position = root_states[:, 0:3]
    ang_velocity = root_states[:, 10:13]

    # Calculate the forward direction cosine similarity with the target direction
    to_target = targets - torso_position
    to_target_norm = torch.norm(to_target, p=2, dim=-1, keepdim=True) + 1e-6
    velocity_norm = torch.norm(velocity[:, 0:2], p=2, dim=-1, keepdim=True) + 1e-6
    cosine_similarity = torch.sum((to_target[:, 0:2] / to_target_norm) * (velocity[:, 0:2] / velocity_norm), dim=-1)

    # Reward for forward velocity using cosine similarity for direction alignment
    velocity_reward = velocity[:, 0] * cosine_similarity
    velocity_reward_temp = 0.1
    transformed_velocity_reward = torch.exp(velocity_reward_temp * velocity_reward) - 1.0

    # Penalty for excessive angular velocity to encourage stability
    ang_velocity_penalty = torch.norm(ang_velocity, p=2, dim=-1) * 0.1
    ang_velocity_penalty_temp = 0.1
    transformed_ang_velocity_penalty = -(torch.exp(ang_velocity_penalty_temp * ang_velocity_penalty) - 1.0)

    # Total reward
    reward = transformed_velocity_reward + transformed_ang_velocity_penalty

    # Reward components dictionary
    reward_components = {
        "velocity_reward": transformed_velocity_reward,
        "ang_velocity_penalty": transformed_ang_velocity_penalty
    }
