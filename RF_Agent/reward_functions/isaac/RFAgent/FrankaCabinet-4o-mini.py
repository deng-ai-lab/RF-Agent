...
def compute_reward(franka_grasp_pos: torch.Tensor, drawer_grasp_pos: torch.Tensor, cabinet_dof_pos: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Calculate distance to the drawer
    distance_to_drawer = torch.norm(franka_grasp_pos - drawer_grasp_pos, dim=-1)

    # Define thresholds and temperature parameters
    goal_distance = torch.tensor(0.1, device=franka_grasp_pos.device)  # Closer tolerance for bonus
    proximity_bonus = torch.exp(-distance_to_drawer / 0.15)  # More sensitive proximity reward

    # Goal bonus for successfully opening the drawer
    drawer_opened = cabinet_dof_pos[:, 3]  # Assuming this indicates if the drawer is opened (0 or 1)
    goal_bonus = drawer_opened * torch.tensor(15.0, device=franka_grasp_pos.device)  # Increased weight for the goal bonus
    
    # Soft distance penalty with corrective scaling
    distance_penalty = -torch.clamp(distance_to_drawer - 0.4, min=0.0) * 0.5  # Penalize positions beyond a threshold
    
    # Total reward computation
    total_reward = proximity_bonus + goal_bonus + distance_penalty
