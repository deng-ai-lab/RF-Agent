...
class AntGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.root_states, self.heading_vec, self.up_vec)
...
@torch.jit.script
def compute_reward(root_states: torch.Tensor, heading_vec: torch.Tensor, up_vec: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    # Cosine similarity reward for maintaining the desired heading direction (x-axis)
    desired_heading = torch.tensor([1.0, 0.0, 0.0], device=heading_vec.device).expand_as(heading_vec)
    heading_cos_sim = torch.nn.functional.cosine_similarity(desired_heading, heading_vec, dim=-1)
    heading_reward = heading_weight * (heading_cos_sim * forward_velocity)

    # Stability penalty for velocity in y and z directions
    y_z_velocity_penalty = y_z_penalty_weight * torch.norm(velocity[:, 1:], p=2, dim=-1)

    # Reward for being upright, using the z-component of the up_vec to measure uprightness
    upright_bonus_temp = 2.0
    upright_bonus = upright_weight * torch.exp(upright_bonus_temp * (up_vec[:, 2] - 1.0))

    # Total reward is a combination of forward movement, heading alignment, and uprightness
    total_reward = forward_velocity + heading_reward + upright_bonus - y_z_velocity_penalty
