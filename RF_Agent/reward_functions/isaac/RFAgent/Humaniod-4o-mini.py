...
def compute_reward(root_states: torch.Tensor, dt: float) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    forward_speed = torso_velocity[:, 0]  # Forward direction speed
    upright_posture = root_states[:, 3:7]  # Torso rotation in quaternion
    
    # Stability bonus - encourages maintaining an upright orientation
    stability_bonus = torch.exp(-torch.norm(upright_posture[:, 1:], p=2, dim=-1) * 0.5)  # Increased weight for stability bonus
    
    # Penalty for excessive angle deviation from the upright position
    angle_deviation = torch.norm(upright_posture[:, 1:], p=2, dim=-1)  # Calculate angular deviation
    penalty_angle = torch.clamp(angle_deviation, max=0.5)  # Reduce the max penalty for better learning

    # Improved temperature values for scaling
    temperature_speed = 1.5  # Reduced temperature scaling for speed reward
    temperature_stability = 1.0  # Temperature for stability bonus

    reward_speed = torch.exp(forward_speed / temperature_speed)  # Speed reward
    stability_reward = stability_bonus  # Use the stability bonus directly
    penalty_reward = -penalty_angle  # Apply the penalty

    total_reward = reward_speed + stability_reward + penalty_reward  # Combine rewards
    reward_components = {
        'speed_reward': reward_speed,
        'stability_bonus': stability_reward,
        'penalty_angle': penalty_reward
    }
