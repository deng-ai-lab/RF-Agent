...
def compute_reward(object_rot: torch.Tensor, goal_rot: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Calculate the relative orientation between object and goal
    orientation_difference = quat_mul(object_rot, quat_conjugate(goal_rot))
    
    # Calculate the deviation from perfect alignment (quat should be close to [0, 0, 0, 1])
    alignment_error = 1.0 - torch.abs(orientation_difference[:, -1])  # assuming the last element is the scalar part
    
    # Transform the alignment error to a reward component
    temperature_orientation = 10.0  # temperature for the orientation component
    reward_orientation = torch.exp(-temperature_orientation * alignment_error)
    
    # Bonus for achieving exact alignment up to a small tolerance
    tolerance = 0.01
    goal_bonus = torch.where(alignment_error < tolerance, 1.0, 0.0)
    
    # Total reward
    total_reward = reward_orientation + goal_bonus

    # Return the composed reward and components
    return total_reward, {
