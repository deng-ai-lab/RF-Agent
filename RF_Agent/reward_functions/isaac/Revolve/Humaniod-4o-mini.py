...
class HumanoidGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.root_states, self.dof_vel, self.dt)
...
@torch.jit.script
def compute_reward(root_states: torch.Tensor, dof_vel: torch.Tensor, dt: float) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Parameters
    speed_temperature = 0.1   # Temperature for speed normalization
    upright_temperature = 0.2  # Temperature for upright normalization

    # Extract torso linear velocity components
    torso_velocity = root_states[:, 7:10]  
    speed = torch.norm(torso_velocity[:, :2], p=2, dim=-1)  # Calculate speed in the x-y plane

    # Speed rewards
    speed_reward = speed  # Direct reward for speed
    normalized_speed_reward = torch.exp(speed_temperature * speed_reward)  # Normalized speed reward

    # Modified reward for staying upright considering both roll and pitch
    upright_reward = 1.0 - torch.max(torch.abs(root_states[:, 3]), torch.abs(root_states[:, 4]))  # Penalize when not upright
    normalized_upright_reward = torch.exp(upright_temperature * upright_reward)  # Normalized upright reward

    # Total reward calculation
    total_reward = normalized_speed_reward + speed_reward + normalized_upright_reward
