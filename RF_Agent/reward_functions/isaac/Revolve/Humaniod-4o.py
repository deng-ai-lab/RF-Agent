...
class HumanoidGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.root_states, self.targets, self.potentials, self.prev_potentials, self.dof_vel, self.actions)
...
def compute_reward(root_states: torch.Tensor, targets: torch.Tensor, potentials: torch.Tensor, 
                   prev_potentials: torch.Tensor, dof_vel: torch.Tensor, actions: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Extract necessary components from root_states
    velocity = root_states[:, 7:10]  # Forward velocity is along the x-axis
    torso_up = root_states[:, 2]  # Dimension representing the vertical torso position

    # Forward Velocity Reward
    forward_velocity_temperature = 0.5  # Temperature parameter
    forward_velocity_reward = torch.exp(forward_velocity_temperature * velocity[:, 0]) - 1.0

    # Mutated Reward for Running Fast with adjusted temperature
    velocity_magnitude = torch.norm(velocity[:, :2], p=2, dim=-1)
    temperature_speed = 5.0  # Previously 10.0, adjusted lower for increased sensitivity
    reward_running_fast = torch.exp(velocity_magnitude / temperature_speed) - 1.0

    # Mutated Upright Penalty
    upright_position_tolerance = 0.8  # Adjusted tolerance for better sensitivity
    upright_penalty_temperature = -2.0  # Temperature parameter for penalty sensitivity
    upright_penalty = torch.exp(upright_penalty_temperature * (upright_position_tolerance - torso_up)) - 1.0
    upright_penalty = torch.where(torso_up < upright_position_tolerance, upright_penalty, torch.tensor(0.0, device=root_states.device))

    # Reward for maintaining a forward potential
    reward_potential = potentials - prev_potentials
    temperature_potential = 5.0  # Temperature parameter for potential
    reward_potential_transformed = torch.exp(reward_potential / temperature_potential) - 1.0

    # Total Reward calculation with different weights to balance components
    total_reward = (1.0 * forward_velocity_reward + 
                    1.0 * reward_running_fast + 
                   -1.0 * upright_penalty +
                    1.0 * reward_potential_transformed)

    # Dictionary of reward components
    reward_dict = {
        'forward_velocity_reward': forward_velocity_reward,
        'reward_running_fast': reward_running_fast,
        'upright_penalty': upright_penalty,
        'reward_potential': reward_potential_transformed
    }
