...
def compute_reward(object_rot: torch.Tensor, goal_rot: torch.Tensor, object_angvel: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Compute orientation alignment reward using squared difference for sensitivity
    orientation_diff = (1.0 - torch.abs(torch.sum(object_rot * goal_rot, dim=-1))) ** 2

    # Angular velocity reward to encourage spinning
    angular_velocity_magnitude = torch.norm(object_angvel, dim=-1)

    # Temperature parameters for transformations
    orientation_temp = 4.0  # Increased to enhance sensitivity to orientation differences
    angular_velocity_temp = 3.0  # Kept the same to maintain sensitivity to angular velocity dynamics

    # Transformations for reward components
    orientation_reward = torch.exp(-orientation_temp * orientation_diff)
    velocity_reward = torch.exp(-angular_velocity_temp * (1.0 / (angular_velocity_magnitude + 1e-6)))

    # Total reward
    reward = orientation_reward + velocity_reward

    # Formulating the dictionary of individual reward components
    reward_components = {
