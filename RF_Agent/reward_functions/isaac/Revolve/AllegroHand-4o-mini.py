...
def compute_reward(object_rot: torch.Tensor, goal_rot: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Define temperature for the reward scaling
    orientation_temp = 1.0

    # Calculate the quaternion distance between the current and goal rotation
    dot_product = torch.clamp(torch.sum(object_rot * goal_rot, dim=-1), min=-1.0, max=1.0)
    angle_diff = torch.acos(dot_product)  # Angular distance in radians

    # Reward based on how close the object's orientation is to the goal orientation
    orientation_reward = -angle_diff  # Negative distance for similarity
    normalized_orientation_reward = torch.exp(orientation_temp * orientation_reward)

    # Introducing a weighted transformation of the angle difference to create more variance
    transformed_angle_difference = angle_diff * 0.5  # Adjusting to provide a lighter penalty
    transformed_rotation_reward = torch.exp(orientation_temp * -transformed_angle_difference)

    # Total reward is the sum of both normalized rewards
    total_reward = normalized_orientation_reward + transformed_rotation_reward

    # Create a dictionary for individual reward components
    reward_components = {
        'orientation_reward': normalized_orientation_reward,
        'transformed_rotation_reward': transformed_rotation_reward
    }
