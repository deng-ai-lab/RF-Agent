...
def compute_reward(root_states: torch.Tensor, targets: torch.Tensor, dt: float) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    distance_temperature = 0.3  # Adjusted temperature for distance reward
    ...
    # Reward for moving forward: positive reward for forward speed
    forward_speed_reward = forward_speed * dt

    # Normalize the forward speed reward to a manageable range
    normalized_forward_speed_reward = torch.exp(forward_speed_reward / forward_velocity_temperature)

    # Calculate distance to target
    torso_position = root_states[:, 0:3]
    distance_to_target = torch.norm(targets - torso_position, p=2)
    
    # Reward for getting closer to the target (transformed into a positive reward)
    reward_proximity = 1.0 / (distance_to_target + 1e-6)  # adding a small value to avoid division by zero
    
    # Normalize the proximity reward
    normalized_distance_reward = torch.exp(reward_proximity / distance_temperature)

    # Total reward is a combination of the forward speed reward and the proximity reward
    total_reward = normalized_forward_speed_reward + normalized_distance_reward

    # Create a dictionary for individual reward components
    reward_components = {
        'forward_speed_reward': normalized_forward_speed_reward,
