...
@torch.jit.script
def compute_reward(root_states: torch.Tensor, commands: torch.Tensor, dof_pos: torch.Tensor, default_dof_pos: torch.Tensor, dof_vel: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Compute velocity errors
    base_quat = root_states[:, 3:7]
    base_lin_vel = quat_rotate_inverse(base_quat, root_states[:, 7:10])
    base_ang_vel = quat_rotate_inverse(base_quat, root_states[:, 10:13])
    
    velocity_error = base_lin_vel[:, :2] - commands[:, :2]
    yaw_velocity_error = base_ang_vel[:, 2] - commands[:, 2]
    
    # Adjusted reward components
    velocity_error_reward = -torch.norm(velocity_error, dim=-1)
    yaw_velocity_error_reward = -torch.abs(yaw_velocity_error)

    # Adjusted transformation for the velocity error component with new temperature
    velocity_temp = 2.0  # Further increased temperature for velocity error component
    yaw_velocity_temp = 1.0

    velocity_error_reward_transformed = torch.exp(velocity_temp * velocity_error_reward)
    yaw_velocity_error_reward_transformed = torch.exp(yaw_velocity_temp * yaw_velocity_error_reward)

    # Calculate total reward
    total_reward = velocity_error_reward_transformed + yaw_velocity_error_reward_transformed

    # Output individual reward components
    reward_dict = {
        "velocity_error_reward": velocity_error_reward_transformed,
        "yaw_velocity_error_reward": yaw_velocity_error_reward_transformed
    }
