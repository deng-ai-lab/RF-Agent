...
class AnymalGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.root_states, self.commands)
...
@torch.jit.script
def compute_reward(root_states: torch.Tensor, commands: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Extract the base linear velocities and angular velocity from the root states
    base_lin_vel = root_states[:, 7:10]
    base_ang_vel = root_states[:, 10:13]

    # The target velocities are commands: we assume the commands are [vx, vy, yaw_rate]
    target_lin_vel = commands[:, :2]
    target_ang_vel = commands[:, 2:3]

    # Compute the velocity errors
    linear_vel_error = target_lin_vel - base_lin_vel[:, :2]  # Only consider x and y for linear velocity
    angular_vel_error = target_ang_vel - base_ang_vel[:, 2:3]  # Yaw rate only from angular velocity

    # Define constants for reward shaping
    linear_reward_temp = 1.0  # Temperature for linear reward normalization
    angular_reward_temp = 3.0  # Increased temperature for angular reward normalization
    angular_reward_scale = 10.0  # Scaling factor for angular reward

    # Compute individual reward components
    linear_reward = -torch.square(torch.norm(linear_vel_error, dim=1))  # Squared penalty for linear velocity error
    angular_reward = -angular_reward_scale * torch.square(torch.norm(angular_vel_error, dim=1))  # Squared penalty for yaw rate error, scaled

    # Normalize rewards using exponential function
    normalized_linear_reward = torch.exp(linear_reward_temp * linear_reward)
    normalized_angular_reward = torch.exp(angular_reward_temp * angular_reward)

    # Combine rewards
    total_reward = normalized_linear_reward + normalized_angular_reward

    # Create a dictionary for individual rewards
    reward_components = {
        "linear_reward": normalized_linear_reward,
        "angular_reward": normalized_angular_reward
    }
