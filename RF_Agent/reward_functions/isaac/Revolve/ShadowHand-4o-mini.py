...
class ShadowHandGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_pos, self.object_rot, self.goal_rot)
...
@torch.jit.script
def compute_reward(object_pos: torch.Tensor, object_rot: torch.Tensor, goal_rot: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Temperature variables for reward transformation
    temp_dist = 0.1
    temp_orientation = 0.1

    # Compute the distance reward based on the difference between the object and goal rotations
    rotation_diff = torch.acos(torch.clamp(torch.sum(object_rot * goal_rot, dim=-1), -1.0, 1.0))
    orientation_reward = -rotation_diff  # More negative as the orientation becomes further away

    # Normalizing the orientation reward
    orientation_reward = torch.exp(temp_orientation * orientation_reward)
