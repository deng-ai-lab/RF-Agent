...
def compute_reward(
    ...
    # Calculate the difference in orientation between the object and the goal
    orientation_diff = quat_mul(object_rot, quat_conjugate(goal_rot))
    orientation_reward_temp = 5.0  # Temperature for alignment reward
    orientation_reward = torch.exp(-orientation_reward_temp * torch.norm(orientation_diff[:, :3], dim=-1))

    # Reward stable spinning by rewarding smaller angular velocities
    angvel_stability_reward_temp = 0.1  # Smaller temperature for stability
    angvel_stability_reward = torch.exp(-angvel_stability_reward_temp * torch.norm(object_angvel, dim=-1))

    # Total reward is the sum of the components
    total_reward = orientation_reward + 0.1 * angvel_stability_reward

    # Creating reward dictionary
    reward_dict = {
        "orientation_reward": orientation_reward,
        "angvel_stability_reward": angvel_stability_reward
    }
