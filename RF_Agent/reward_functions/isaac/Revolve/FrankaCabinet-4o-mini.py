...
def compute_reward(franka_grasp_pos: torch.Tensor, drawer_grasp_pos: torch.Tensor, cabinet_dof_pos: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    normalized_distance_reward = torch.exp(distance_reward) * 0.5  # Adjusted scaling factor for distance reward

    drawer_opened = cabinet_dof_pos[:, 3] > distance_threshold
    goal_bonus = torch.where(drawer_opened, torch.tensor(3.0, device=franka_grasp_pos.device), torch.tensor(0.0, device=franka_grasp_pos.device))  # Increased goal bonus
    
    total_reward = normalized_distance_reward + goal_bonus

    return total_reward, {'normalized_distance_reward': normalized_distance_reward, 'goal_bonus': goal_bonus}
