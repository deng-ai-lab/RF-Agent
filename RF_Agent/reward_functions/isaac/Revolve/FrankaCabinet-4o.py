...
class FrankaCabinetGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.franka_grasp_pos, self.drawer_grasp_pos, self.cabinet_dof_pos, self.cabinet_dof_vel)
...
def compute_reward(
    ...
    # Compute the distance from the gripper to the target position on the drawer
    distance_to_drawer = torch.norm(franka_grasp_pos - drawer_grasp_pos, dim=-1)
    
    # Use exponential transformation for enhanced learning feedback
    distance_reward_weight = 1.0
    distance_reward_temperature = 0.15
    distance_reward_raw = -distance_to_drawer * distance_reward_weight
    distance_reward = torch.exp(distance_reward_raw / distance_reward_temperature)

    # Compute the reward for the cabinet being more opened
    cabinet_open_reward_weight = 2.0
    cabinet_open_reward = cabinet_dof_pos[:, 3] * cabinet_open_reward_weight

    # Reward for the velocity of opening
    opening_velocity_reward_weight = 0.4  # Reduced weight for stability
    opening_velocity_reward = cabinet_dof_vel[:, 3] * opening_velocity_reward_weight

    # Total weighted reward
    total_reward = distance_reward + cabinet_open_reward + opening_velocity_reward

    # Normalize and apply an exponential transformation to boost large improvements
    total_reward_temperature = 0.1
    total_reward = torch.exp(total_reward / total_reward_temperature)

    # Compile reward components for analysis
    reward_components = {
