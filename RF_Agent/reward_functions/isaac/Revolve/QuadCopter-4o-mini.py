...
def compute_reward(root_positions: torch.Tensor, root_linvels: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    target_position = torch.tensor([0.0, 0.0, 1.0], dtype=root_positions.dtype, device=root_positions.device)

    # Compute the distance to the target position
    distance_to_target = torch.norm(root_positions - target_position, dim=-1)
    position_reward = 1.0 / (1.0 + distance_to_target)

    # Adjusted reward for low linear velocity (to encourage hovering)
    velocity_magnitude = torch.norm(root_linvels, dim=-1)
    hovering_reward = -velocity_magnitude ** 2  # Emphasize penalties for high velocities

    # Temperature parameters for transforming rewards
    position_temp = 10.0  # High sensitivity for position reward
    hovering_temp = 0.05  # Moderate sensitivity for hovering

    # Transform the rewards to normalize them
    transformed_position_reward = position_temp * position_reward
    transformed_hovering_reward = torch.exp(hovering_temp * hovering_reward)

    # Total reward
    total_reward = transformed_position_reward + transformed_hovering_reward

    # Create a dictionary of individual reward components
    reward_components = {
        'position_reward': transformed_position_reward,
        'hovering_reward': transformed_hovering_reward,
    }
