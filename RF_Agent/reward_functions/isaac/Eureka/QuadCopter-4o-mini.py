...
def compute_reward(root_positions: torch.Tensor, root_linvels: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Define fixed target position
    target_pos = torch.tensor([0.0, 0.0, 1.0], device=root_positions.device)

    # Calculate distance to target
    distance_to_target = torch.norm(root_positions - target_pos, dim=-1)

    # Distance reward - penalize distance in a quadratic manner
    distance_reward = -1 * (distance_to_target ** 2)  # Quadratic penalty for distance

    # Velocity penalty - apply a sharper penalty for exceeding a threshold
    velocity_threshold = 0.5  # Define a threshold for acceptable speed
    excess_velocity = torch.norm(root_linvels, dim=-1) - velocity_threshold
    velocity_penalty = -torch.where(excess_velocity > 0,
                                     excess_velocity ** 2,  # Quadratic penalty on excess velocity
                                     torch.tensor(0.0, device=root_positions.device))

    # Stability reward - ensure it's normalized and lower the scale
    stability_reward_radius = 0.15  # A small radius for stability
    stability_reward = 5 * torch.where(distance_to_target < stability_reward_radius,
                                        torch.tensor(1.0, device=root_positions.device),
                                        torch.tensor(0.0, device=root_positions.device))

    # Total reward is a combination of these components
    reward = distance_reward + velocity_penalty + stability_reward

    # Transform each component for consistency in ranges
    temperature_distance = 0.5
    temperature_velocity = 0.3
    temperature_stability = 0.4
    
    transformed_distance_reward = torch.exp(temperature_distance * distance_reward)
    transformed_velocity_penalty = torch.exp(temperature_velocity * velocity_penalty)
    transformed_stability_reward = torch.exp(temperature_stability * stability_reward)

    # Formulate the total reward by combining transformed rewards
    total_reward = transformed_distance_reward + transformed_velocity_penalty + transformed_stability_reward

    # Create individual reward components dictionary
    individual_rewards = {
        'distance_reward': transformed_distance_reward,
        'velocity_penalty': transformed_velocity_penalty,
