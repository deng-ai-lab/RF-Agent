...
def compute_reward(root_states: torch.Tensor, dt: float, targets: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    """
    Compute the reward based on the humanoid's speed and completion of the task.

    Parameters:
    - root_states: A tensor containing the root state of the humanoid including positions and velocities.
    - dt: A float representing the time step.
    - targets: A tensor representing the target positions for the humanoid.

    Returns:
    - A tuple containing the total reward and a dictionary of individual reward components.
    """
    # Extract velocity from the root states
    ...
    # Speed reward component with a modified temperature
    ...
    # Task score reward: encourage minimizing distance to target
    temperature_task = 0.2
    task_score_reward = torch.exp(-distance_to_target / temperature_task)

    # Penalty for falling (considering a simple z-component threshold)
    z_threshold = 0.5  # Define a threshold for falling
    falling_penalty = torch.where(torso_position[:, 2] < z_threshold, torch.tensor(-10.0, device=root_states.device), torch.tensor(0.0, device=root_states.device))

    # Total reward integrating all components
    total_reward = speed_reward + task_score_reward + falling_penalty
    
    # Normalization
    total_reward = torch.clamp(total_reward, min=-10, max=100)
