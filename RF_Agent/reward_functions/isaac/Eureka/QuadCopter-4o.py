...
class QuadcopterGPT(VecTask):
    ...
    def compute_reward(self):
        self.rew_buf[:], self.rew_dict = compute_reward(self.root_positions, self.root_quats, self.root_linvels, self.root_angvels)
...
@torch.jit.script
def compute_reward(root_positions: torch.Tensor, root_quats: torch.Tensor, root_linvels: torch.Tensor, root_angvels: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    # Rewritten stability reward focusing on level orientation with respect to the horizontal plane
    tilt_angle = torch.acos(torch.clamp(root_quats[..., 2], -1.0, 1.0))  # Assuming the dot product of up vector and z-axis gives the cosine of the tilt angle
    stability = torch.abs(tilt_angle)

    # Linear and angular velocity penalties for smoother hovering
    linear_velocity_penalty = torch.norm(root_linvels, dim=-1)
    angular_velocity_penalty = torch.norm(root_angvels, dim=-1)

    # Reward Calculations
    distance_reward_temperature = 4.0
    distance_reward = torch.exp(-distance_to_target * distance_reward_temperature)

    stability_reward_temperature = 10.0
    stability_reward = torch.exp(-stability * stability_reward_temperature)

    velocity_penalty_temperature = 3.0
    velocity_reward = torch.exp(-(linear_velocity_penalty + angular_velocity_penalty) * velocity_penalty_temperature)

    # Adjusted rewards contributing to the total
    total_reward = distance_reward + stability_reward + velocity_reward
