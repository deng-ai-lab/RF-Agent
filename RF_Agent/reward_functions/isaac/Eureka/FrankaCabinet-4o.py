...
def compute_reward(franka_grasp_pos: torch.Tensor, drawer_grasp_pos: torch.Tensor, 
    ...
    # Recalculate distance to drawer for reward
    distance_to_drawer = torch.norm(drawer_grasp_pos - franka_grasp_pos, dim=-1)
    # Encourage moving closer with a higher positive reward
    transformed_distance_reward = torch.exp(-distance_to_drawer / distance_temp)

    # Reward for opening the cabinet door
    cabinet_opening_reward = cabinet_dof_pos[:, 3]
    # Rescale the opening reward
    transformed_opening_reward = torch.exp(cabinet_opening_reward / opening_temp)

    # Combine the transformed rewards with normalized scaling
    distance_reward_weight = 1.0
    opening_reward_weight = 1.0
    total_reward = distance_reward_weight * transformed_distance_reward + opening_reward_weight * transformed_opening_reward

    # Dictionary of individual reward components
    reward_components = {
        "transformed_distance_reward": transformed_distance_reward,
        "transformed_opening_reward": transformed_opening_reward,
        "distance_to_drawer": distance_to_drawer,
        "cabinet_opening_reward": cabinet_opening_reward,
    }
