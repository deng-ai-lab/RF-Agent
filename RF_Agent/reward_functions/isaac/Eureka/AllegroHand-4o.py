...
def compute_reward(object_rot: torch.Tensor,
    ...
    orientation_alignment = 1.0 - torch.sum(object_rot * goal_rot, dim=-1)
    
    # Calculate angular velocity magnitude
    angular_velocity_magnitude = torch.norm(object_angvel, dim=-1)

    # Introduce distinct temperatures for rewards
    orientation_temp = 1.0  # Increase to sharpen the effect of achieving precise orientation
    velocity_temp = 1.0    # Increased to better penalize unwanted spinning

    # Exponential transformations for better control
    orientation_reward = torch.exp(-orientation_temp * orientation_alignment)
    angular_velocity_penalty = torch.exp(-velocity_temp * angular_velocity_magnitude)

    # New angular velocity penalty with increasing penalty for high velocity
    stabilizing_penalty_temp = 2.0
    stabilizing_penalty = torch.exp(-stabilizing_penalty_temp * torch.clip(angular_velocity_magnitude - 0.1, min=0))

    # Reweight the components
    total_reward = (2.0 * orientation_reward) - (1.0 * angular_velocity_penalty) - (2.0 * stabilizing_penalty)

    # Construct individual component dictionary for insights
    reward_dict = {
        "orientation_reward": orientation_reward,
        "angular_velocity_penalty": angular_velocity_penalty,
        "stabilizing_penalty": stabilizing_penalty
    }
