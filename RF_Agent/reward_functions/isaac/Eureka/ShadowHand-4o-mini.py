...
def compute_reward(object_rot: torch.Tensor, goal_rot: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    temperature_progress = 0.5
    temperature_completion = 0.1
    temperature_task_score = 0.25

    # Calculate angular difference using the dot product of quaternions
    rot_diff = torch.acos(torch.clamp(torch.sum(object_rot * goal_rot, dim=-1), -1.0, 1.0))

    # Progress reward with inverse scaling
    progress_reward = 1.0 - (rot_diff / torch.pi)  # Scale between 0 and 1
    normalized_progress_reward = torch.exp(temperature_progress * progress_reward) - 1.0  # Transformation

    # Completion reward based on squared angular difference
    completion_reward = torch.exp(- (rot_diff ** 2 / (torch.pi ** 2)))  # Ensure high sensitivity
    normalized_completion_reward = torch.exp(temperature_completion * completion_reward) - 1.0  # Transformation

    # Task score, added to keep effective completions
    task_score = (rot_diff < 0.1).float()  # 1 if completed, else 0
    normalized_task_score = torch.exp(temperature_task_score * task_score) - 1.0  # Transformation

    # Total reward combining all components
    total_reward = normalized_progress_reward + normalized_completion_reward + normalized_task_score

    # Create a dictionary to hold individual rewards
    reward_components = {
        "progress_reward": normalized_progress_reward,
        "completion_reward": normalized_completion_reward,
