...
class FrankaCabinetGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.rigid_body_states, self.drawer_handle, self.hand_handle, self.franka_grasp_pos, self.drawer_grasp_pos)
...
@torch.jit.script
def compute_reward(rigid_body_states: torch.Tensor, drawer_handle: int, hand_handle: int, 
                   franka_grasp_pos: torch.Tensor, drawer_grasp_pos: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Constants
    distance_threshold = 0.1  # considered 'opened' proximity
    temperature_position = 0.05  # new temperature for position-based rewards
    temperature_open = 1.0  # temperature for open reward scaling
    close_enough_reward = 0.5  # reward for being close to open
    bonus_reward_if_opened = 1.0  # reward for actually 'opened' cabinet
    
    # Extract positions
    hand_pos = rigid_body_states[:, hand_handle][:, 0:3]
    drawer_pos = rigid_body_states[:, drawer_handle][:, 0:3]
    
    # Compute distances
    distance_to_drawer = torch.norm(drawer_pos - hand_pos, dim=-1)

    # Reward for proximity to drawer (decay as distance increases)
    position_reward = close_enough_reward * torch.exp(-temperature_position * distance_to_drawer)

    # Reward for being within 'opened' proximity
    is_opened = distance_to_drawer < distance_threshold
    open_reward = is_opened.to(hand_pos.dtype) * bonus_reward_if_opened

    # Total reward
    total_reward = position_reward + open_reward
