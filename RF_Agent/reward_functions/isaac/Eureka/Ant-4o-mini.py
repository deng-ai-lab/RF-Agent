...
def compute_reward(root_states: torch.Tensor, targets: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    forward_velocity_temperature = torch.tensor(0.1, dtype=root_states.dtype, device=root_states.device)
    reaching_goal_temperature = torch.tensor(0.01, dtype=root_states.dtype, device=root_states.device)
    stability_temperature = torch.tensor(0.03, dtype=root_states.dtype, device=root_states.device)

    # Extract the torso velocity (forward motion)
    torso_velocity = root_states[:, 7:10]  # Assuming column 7-9 contains x, y, z velocity
    forward_velocity = torso_velocity[:, 0]  # Forward motion assumed along the x-axis

    # Calculate individual rewards
    forward_reward = torch.exp(forward_velocity * forward_velocity_temperature)

    # Calculate the distance to the target and provide a reward for being close (revised goal reward)
    torso_position = root_states[:, 0:3]
    distance_to_target = torch.norm(targets - torso_position, p=2, dim=-1)
    reaching_goal_reward = torch.exp(-distance_to_target * reaching_goal_temperature)

    # Reward for staying upright or minimizing instability (z velocity can be used)
    upright_stability = torch.abs(torso_velocity[:, 2])  # Assuming a non-negative z velocity is a good proxy for being upright
    stability_reward = torch.exp(-upright_stability * stability_temperature)

    # Additional reward for distance traveled (to encourage continuous forward movement)
    traveled_distance = torch.norm(torso_velocity[:, :2], p=2, dim=-1)  # Modify to consider x, y motion
    distance_reward = torch.exp(traveled_distance * forward_velocity_temperature)

    # Total reward
    total_reward = forward_reward + reaching_goal_reward + stability_reward + distance_reward

    # Create a dictionary of rewards
    reward_components = {
        'forward_reward': forward_reward,
        'reaching_goal_reward': reaching_goal_reward,
