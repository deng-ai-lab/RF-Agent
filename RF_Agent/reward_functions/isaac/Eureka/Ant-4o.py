...
class AntGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.root_states, self.potentials, self.prev_potentials, self.actions, self.dof_vel, self.dof_limits_lower, self.dof_limits_upper)
...
@torch.jit.script
def compute_reward(root_states: torch.Tensor, potentials: torch.Tensor, prev_potentials: torch.Tensor, actions: torch.Tensor, 
                   dof_vel: torch.Tensor, dof_limits_lower: torch.Tensor, dof_limits_upper: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    forward_velocity_reward = velocity[:, 0]  # Assuming forward is along the x-axis
    ...
    exp_target_closeness_reward = torch.exp(target_closeness_reward_temperate * target_closeness_reward)
    ...
    total_reward = forward_velocity_reward + 0.5 * exp_target_closeness_reward + 0.1 * exp_dof_vel_penalty + 0.1 * exp_actions_penalty
