...
def compute_reward(root_states: torch.Tensor, commands: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Define temperature variables for the transformation
    position_temperature: float = 0.05
    yaw_temperature: float = 0.1

    # Extract linear and angular velocities from root_states
    base_lin_vel = root_states[:, 7:10]  # Linear velocity
    base_ang_vel = root_states[:, 10:13]  # Angular velocity

    # Calculate velocity errors
    velocity_error = commands - base_lin_vel
    position_reward = 1.0 - (torch.norm(velocity_error, dim=-1) / (torch.norm(commands, dim=-1) + 1e-6))

    # Calculate yaw error and reformulate yaw reward
    yaw_error = commands[:, 2] - base_ang_vel[:, 2]
    yaw_reward = torch.exp(-torch.abs(yaw_error))  # Positive reinforcement for yaw

    # Scale the position reward to bring it in line with yaw reward magnitude
    position_reward_scaled = position_reward * 0.2  # Scale down

    # Transform rewards
    transformed_position_reward = torch.exp(position_reward_scaled / position_temperature) - 1.0
    transformed_yaw_reward = yaw_reward  # Keep yaw reward

    # Total reward
    total_reward = transformed_position_reward + transformed_yaw_reward

    # Individual reward components for analysis
    individual_rewards = {
        'position_reward': transformed_position_reward,
        'yaw_reward': transformed_yaw_reward,
    }
