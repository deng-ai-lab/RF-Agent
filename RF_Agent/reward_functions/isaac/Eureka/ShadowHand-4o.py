...
def compute_reward(object_rot: torch.Tensor, goal_rot: torch.Tensor, object_angvel: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Rotation alignment reward: incentivize matching object rotation with goal rotation
    rotation_diff = torch.abs(torch.sum(object_rot * goal_rot, dim=-1))
    rotation_temperature = 9.0  # Elevate emphasis on perfect alignment
    rotation_reward_transformed = torch.exp(rotation_temperature * (rotation_diff - 1.0))

    # Angular velocity penalty: less negative to support controlled object spinning
    angular_velocity_norm = torch.norm(object_angvel, dim=-1)
    angular_velocity_temperature = 0.3  # Mild penalty adjustment
    angular_velocity_penalty_transformed = -torch.exp(-angular_velocity_temperature * angular_velocity_norm)

    # Total reward combines a more significant rotation influence with measured velocity control
    total_reward = rotation_reward_transformed + angular_velocity_penalty_transformed

    reward_components = {
        "rotation_reward": rotation_reward_transformed,
