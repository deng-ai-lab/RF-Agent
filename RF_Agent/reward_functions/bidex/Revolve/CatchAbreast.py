...
class ShadowHandCatchAbreastGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_pos, self.goal_pos, self.left_hand_pos, self.right_hand_pos, self.left_hand_rot, self.right_hand_rot)
...
@torch.jit.script
def compute_reward(object_pos: torch.Tensor, goal_pos: torch.Tensor, left_hand_pos: torch.Tensor, right_hand_pos: torch.Tensor, left_hand_rot: torch.Tensor, right_hand_rot: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    distance_to_left_hand = torch.norm(object_pos - left_hand_pos, dim=-1) + 1e-6  # Adding a small epsilon to prevent log(0)
    distance_to_right_hand = torch.norm(object_pos - right_hand_pos, dim=-1) + 1e-6
    
    # Encourage the object to be close to the hands with stronger feedback
    reward_catch_left_base = -torch.exp(distance_to_left_hand / temp_catch)  # Base feedback
    reward_catch_left_prox = torch.where(distance_to_left_hand < 0.1, 1.0 - distance_to_left_hand / 0.1, torch.tensor(0.0, device=object_pos.device))  # Linear boost for proximity
    reward_catch_left = reward_catch_left_base + reward_catch_left_prox  # Combined rewards

    reward_catch_right = -1.5 * torch.exp(distance_to_right_hand / temp_catch)  # More weight for right hand penalty
    reward_catch = reward_catch_left + reward_catch_right  # Combined rewards for better guidance
    ...
    reward_left_rot = -torch.abs(left_hand_rot[:, 0] - desired_left_rot)  # Compare some angle derived from quaternion
    reward_right_rot = -torch.abs(right_hand_rot[:, 0] - desired_right_rot)

    # Combine all rewards
    total_reward = torch.exp(reward_position / temp_pos) + torch.exp(reward_catch / temp_catch) + torch.exp(reward_left_rot / temp_rot) + torch.exp(reward_right_rot / temp_rot)
