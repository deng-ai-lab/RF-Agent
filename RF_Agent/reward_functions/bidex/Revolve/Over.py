...
class ShadowHandOverGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_pos, self.goal_pos, self.fingertip_pos, self.fingertip_another_pos, self.object_linvel, self.object_angvel)
...
def compute_reward(object_pos: torch.Tensor, goal_pos: torch.Tensor,
                   ...
                   object_linvel: torch.Tensor, object_angvel: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    
    # Define temperature values
    distance_temp = 0.5  # Moderate influence for distance
    height_temp = 1.0    # Standard temperature for height reward
    velocity_temp = 4.0   # Increased emphasis on positive velocity contribution
    negative_velocity_penalty_temp = 1.5  # Penalizes negative contributions, but less emphasis

    # Calculate distance between the object and the goal position
    distance = torch.norm(object_pos - goal_pos, dim=-1)
    distance_reward = -(distance**2) + object_pos[:, 2]  # Incorporates height

    # Height reward based on object's height (z-coordinate)
    height_reward = object_pos[:, 2]

    # Calculate the directional vector towards the goal
    target_direction = goal_pos - object_pos
    target_direction_norm = target_direction / (torch.norm(target_direction, dim=-1, keepdim=True) + 1e-6)  # Avoid division by zero
    velocity_projection = torch.sum(object_linvel * target_direction_norm, dim=-1)  # Project velocity towards the goal
    
    # Clamping for positive contributions
    max_velocity = 1.0   
    velocity_reward = torch.clamp(velocity_projection / max_velocity, min=0, max=1)  # Scale to [0,1]

    # Include a penalty for negative or zero velocities
    negative_velocity_penalty = torch.clamp(-velocity_projection / max_velocity, min=0, max=1)  # Penalizes negative contribution
    
    # Combine rewards with normalization transformations
    total_reward = (
        torch.exp(distance_temp * distance_reward) + 
        torch.exp(height_temp * height_reward) + 
        torch.exp(velocity_temp * velocity_reward) - 
        torch.exp(negative_velocity_penalty_temp * negative_velocity_penalty)
    )

    # Construct individual reward components
    reward_components = {
