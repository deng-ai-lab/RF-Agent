...
def compute_reward(object_pos: torch.Tensor, door_left_handle_pos: torch.Tensor, door_right_handle_pos: torch.Tensor, 
    ...
    # Define temperature variables for reward scaling
    handle_reach_temp = 0.5
    door_closed_temp = 0.1  # Adjust the temperature for door closed reward
    goal_distance_threshold = 0.5  # Define threshold for closed door reward
    ...
    # Reward components
    handle_reach_reward = -torch.clamp(torch.minimum(left_handle_distance, right_handle_distance), min=0.0)  # Closer is better
    door_closed_reward = torch.where(goal_distance < goal_distance_threshold,
                                      1.0 - goal_distance / goal_distance_threshold,  # Reward for being near goal
                                      torch.tensor(0.0, device=goal_distance.device))  # No reward if not close

    # Transform rewards to bring them into a suitable range
    handle_reach_reward = torch.exp(handle_reach_temp * handle_reach_reward)
    door_closed_reward = torch.exp(door_closed_reward / door_closed_temp)

    # Total reward is the sum of individual rewards
    total_reward = handle_reach_reward + door_closed_reward

    # Create a dictionary to return each component
    reward_components = {
        "handle_reach_reward": handle_reach_reward,
        "door_closed_reward": door_closed_reward
    }
