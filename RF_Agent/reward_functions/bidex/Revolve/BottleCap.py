...
class ShadowHandBottleCapGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_pos, self.goal_pos, self.left_hand_pos, self.right_hand_pos, self.bottle_cap_pos, self.bottle_pos)
...
def compute_reward(object_pos: torch.Tensor, goal_pos: torch.Tensor, 
                    ...
                    bottle_cap_pos: torch.Tensor, bottle_pos: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Define temperature parameters for reward transformations
    temperature_position = 0.1
    temperature_distance = 0.5
    temperature_grasp = 0.1

    # Reward for positioning the hands close to the bottle cap
    left_hand_to_cap_dist = torch.norm(left_hand_pos - bottle_cap_pos, dim=1)
    right_hand_to_cap_dist = torch.norm(right_hand_pos - bottle_cap_pos, dim=1)
    position_reward = torch.exp(-temperature_position * (left_hand_to_cap_dist + right_hand_to_cap_dist))

    # Check if both hands are close to the cap (grasping condition)
    left_hand_grasp = torch.norm(bottle_cap_pos - left_hand_pos, dim=1) < 0.1
    right_hand_grasp = torch.norm(bottle_cap_pos - right_hand_pos, dim=1) < 0.1
    grasp_condition = left_hand_grasp & right_hand_grasp

    # Reward for grasping the bottle cap with both hands
    reward_grasp = grasp_condition.float()
    reward_grasp = torch.exp(reward_grasp / temperature_grasp)  # Normalize reward

    # Reward for maintaining the hold on the bottle
    bottle_cap_grasp_reward = torch.exp(-temperature_grasp * (torch.norm(left_hand_pos - bottle_pos, dim=1) + torch.norm(right_hand_pos - bottle_pos, dim=1)))
    
    # Total reward is a combination of all components
    total_reward = position_reward * 0.3 + reward_grasp * 0.4 + bottle_cap_grasp_reward * 0.3

    # Create a dictionary for individual reward components
    reward_components = {
        "position_reward": position_reward,
        "reward_grasp": reward_grasp,
        "bottle_cap_grasp_reward": bottle_cap_grasp_reward,
    }
