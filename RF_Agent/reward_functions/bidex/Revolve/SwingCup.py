...
class ShadowHandSwingCupGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_pos, self.goal_pos, self.left_hand_pos, self.right_hand_pos, self.cup_right_handle_pos, self.cup_left_handle_pos)
...
def compute_reward(object_pos: torch.Tensor, goal_pos: torch.Tensor, 
                   ...
                   cup_right_handle_pos: torch.Tensor, cup_left_handle_pos: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    # Grasp reward calculation
    distance_left_handle = torch.norm(left_hand_pos - cup_left_handle_pos, dim=-1)
    distance_right_handle = torch.norm(right_hand_pos - cup_right_handle_pos, dim=-1)
    
    grasp_threshold = 0.1
    max_grasp_reward = 10.0
    grasp_reward_left = max_grasp_reward * torch.sigmoid((grasp_threshold - distance_left_handle) / temp_grasp)
    grasp_reward_right = max_grasp_reward * torch.sigmoid((grasp_threshold - distance_right_handle) / temp_grasp)
    grasp_reward = grasp_reward_left + grasp_reward_right

    # Holding reward calculation
    holding_reward = (torch.where(distance_left_handle < grasp_threshold,
                                   (1 - distance_left_handle / grasp_threshold),
                                   torch.tensor(0.0, device=object_pos.device)) +
                     torch.where(distance_right_handle < grasp_threshold,
                                   (1 - distance_right_handle / grasp_threshold),
                                   torch.tensor(0.0, device=object_pos.device)))
    holding_reward = 5.0 * torch.sigmoid(holding_reward / temp_holding)

    # Distance to goal reward calculation
    distance_to_goal = torch.norm(object_pos - goal_pos, dim=1)
    reward_distance = torch.exp(-distance_to_goal / temp_distance)

    # Total reward
    total_reward = grasp_reward + holding_reward + reward_distance
    
    # Create individual reward dictionary
    reward_components = {
