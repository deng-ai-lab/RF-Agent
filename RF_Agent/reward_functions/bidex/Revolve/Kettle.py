...
class ShadowHandKettleGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_pos, self.goal_pos, self.kettle_handle_pos, self.bucket_handle_pos)
...
@torch.jit.script
def compute_reward(object_pos: torch.Tensor, goal_pos: torch.Tensor, kettle_handle_pos: torch.Tensor, 
                   bucket_handle_pos: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Parameters for reward calculations
    distance_threshold = 0.1
    temperature_position = 5.0
    temperature_goal = 4.0  # Adjusted to allow better optimization

    # Compute distances
    distance_kettle_bucket = torch.norm(bucket_handle_pos - kettle_handle_pos, dim=1)
    distance_to_goal = torch.norm(goal_pos - object_pos, dim=1)

    # High-performing reward component for handles proximity
    reward_kettle_bucket = torch.exp(-distance_kettle_bucket / temperature_position)

    # Improved scaling for the goal distance component
    reward_to_goal = torch.clamp(-1.5 * distance_to_goal / (distance_threshold + 1e-6), min=0.0)

    # Total reward combining the two components
    total_reward = reward_kettle_bucket + reward_to_goal

    # Create a dictionary for individual rewards
    reward_components = {
        'reward_kettle_bucket': reward_kettle_bucket,
