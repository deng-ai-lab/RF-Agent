...
@torch.jit.script
def compute_reward(object_pos: torch.Tensor, block_pos: torch.Tensor, goal_pos: torch.Tensor, left_hand_pos: torch.Tensor, right_hand_pos: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Hyperparameters for reward components
    dist_temp = 0.05  # Distance temperature for goal
    stacking_temp = 0.2  # Increased stacking temperature for better differentiation
    hand_dist_temp = 0.1  # Hand distance temperature
    stacking_success_bonus = 1.0  # Bonus for successful stacking
    
    # Calculate distances
    distance_to_goal = torch.norm(object_pos - goal_pos, dim=-1)
    stacking_distance = torch.norm(object_pos - block_pos, dim=-1)
    hands_distance = torch.norm(left_hand_pos - right_hand_pos, dim=-1)

    # Define reward components
    distance_reward = torch.exp(-dist_temp * distance_to_goal)  # Normalized distance reward
    stacking_reward = stacking_temp * (0.05 - stacking_distance).clamp(min=0)  # Linear increase in stacking reward based on distance
    
    # Condition to give a bonus reward for successful stacking
    stacking_bonus = torch.where(stacking_distance < 0.05, stacking_success_bonus, torch.tensor(0.0, device=object_pos.device))
    stacking_reward += stacking_bonus

    hands_proximity_reward = torch.exp(-hand_dist_temp * hands_distance)  # Normalized hands proximity reward

    # Total reward aggregation
    total_reward = distance_reward + stacking_reward + hands_proximity_reward
