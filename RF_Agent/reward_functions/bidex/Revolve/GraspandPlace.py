...
def compute_reward(object_pos: torch.Tensor, goal_pos: torch.Tensor, 
    ...
    distance_temperature: float = 0.1       # temperature for distance reward
    grasp_temperature: float = 0.5           # updated temperature for grasping reward
    placing_temperature: float = 0.03        # temperature for placing reward

    # Calculate distances
    distance_to_goal = torch.norm(object_pos - goal_pos, dim=1)

    # Reward for getting closer to the goal
    distance_reward = torch.exp(-distance_temperature * distance_to_goal)

    # Reward for grasping: smooth proximity of hands to the object
    ...
    min_distance_to_hands = torch.minimum(distance_to_left_hand, distance_to_right_hand)

    grasp_reward = torch.clamp(1 - (min_distance_to_hands / 0.1), min=0.0, max=1.0) * torch.exp(grasp_temperature)

    # Quadratic placing reward based on proximity to the goal (transformed)
    placing_reward = -torch.pow(distance_to_goal, 2)
    placing_reward_transformed = torch.exp(placing_reward / placing_temperature)

    # Total reward combining distance, grasping, and placing rewards
    total_reward = distance_reward + grasp_reward + placing_reward_transformed

    # Prepare reward components for the output dictionary
    reward_components = {
        'distance_reward': distance_reward,
        'grasp_reward': grasp_reward,
        'placing_reward': placing_reward_transformed
    }
