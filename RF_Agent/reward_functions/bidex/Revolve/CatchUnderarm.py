...
class ShadowHandCatchUnderarmGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_pos, self.object_linvel, self.fingertip_pos, self.goal_pos)
...
@torch.jit.script
def compute_reward(object_pos: torch.Tensor, object_linvel: torch.Tensor, fingertip_pos: torch.Tensor, goal_pos: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    temperature_goal = 0.2
    temperature_upward = 0.5
    temperature_fingertip = 0.5

    # Compute distance to goal and goal reward
    distance_to_goal = torch.norm(object_pos - goal_pos, dim=-1)
    goal_reward = -distance_to_goal
    goal_reward = torch.exp(temperature_goal * goal_reward)

    # Compute upward throw reward with normalization
    upward_velocity = object_linvel[:, 2]
    upward_throw_reward = torch.max(torch.tensor(0.0, device=object_pos.device), upward_velocity)
    upward_throw_reward /= (torch.max(torch.tensor(1.0, device=object_pos.device), upward_velocity.abs().max() + 1e-5))
    upward_throw_reward = torch.exp(temperature_upward * upward_throw_reward)

    # Compute distance to fingertips for penalty with scaling
    distance_to_fingertips = torch.norm(object_pos[:, None, :] - fingertip_pos[:, :, :], dim=-1)
    min_distance = distance_to_fingertips.min(dim=-1)[0]
    fingertip_penalty = torch.exp(-temperature_fingertip * min_distance / (torch.tensor(1.0, device=object_pos.device) + min_distance))

    # Total reward calculation
    total_reward = goal_reward * upward_throw_reward - fingertip_penalty

    # Normalize total reward to [0, 1]
    total_reward = (total_reward - total_reward.min()) / (total_reward.max() - total_reward.min() + 1e-5)

    reward_components = {
        'goal_reward': goal_reward,
        'upward_throw_reward': upward_throw_reward,
        'fingertip_penalty': fingertip_penalty
    }
