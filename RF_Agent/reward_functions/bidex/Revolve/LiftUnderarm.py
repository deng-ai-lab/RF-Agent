...
class ShadowHandLiftUnderarmGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_pos, self.goal_pos, self.left_hand_pos, self.right_hand_pos, self.pot_left_handle_pos, self.pot_right_handle_pos, self.object_linvel)
...
def compute_reward(object_pos: torch.Tensor, goal_pos: torch.Tensor, 
                   ...
                   pot_left_handle_pos: torch.Tensor, pot_right_handle_pos: torch.Tensor, 
    ...
    grasp_temp = 0.1
    lift_temp = 0.005  # Keep the same temperature for lift reward sensitivity
    goal_reward_temp = 0.1  # Temperature for goal reward transformation

    # Calculate distances for rewards
    left_grasp_distance = torch.norm(left_hand_pos - pot_left_handle_pos, dim=-1)
    right_grasp_distance = torch.norm(right_hand_pos - pot_right_handle_pos, dim=-1)
    lift_distance = torch.norm(object_pos - goal_pos, dim=-1)
    
    # Reward for grasping both handles (the closer, the better)
    grasp_left_reward = torch.exp(-left_grasp_distance / grasp_temp)
    grasp_right_reward = torch.exp(-right_grasp_distance / grasp_temp)
    
    # Calculate the direction to the goal and normalize it
    goal_direction = (goal_pos - object_pos) / (torch.norm(goal_pos - object_pos, dim=-1, keepdim=True) + 1e-6)
    lift_speed_reward = torch.sum(object_linvel * goal_direction, dim=-1) * 10.0  # Increased weighting on velocity for stronger positive reinforcement
    downward_penalty = torch.relu(-lift_speed_reward) * 1.0  # Controlled penalty for negative velocity
    lift_reward = torch.exp(-lift_distance / lift_temp) + torch.relu(lift_speed_reward) * 3.0 - downward_penalty  # Amplified positive influence & controlled penalty for downward speeds

    # Reward for reaching the goal
    goal_distance = torch.norm(object_pos - goal_pos, dim=-1)
    goal_reached = torch.exp(-goal_distance / goal_reward_temp)

    # Combine rewards
    total_reward = grasp_left_reward + grasp_right_reward + lift_reward + goal_reached

    # Create a dictionary of individual components
    individual_rewards = {
        'grasp_left_reward': grasp_left_reward,
        'grasp_right_reward': grasp_right_reward,
