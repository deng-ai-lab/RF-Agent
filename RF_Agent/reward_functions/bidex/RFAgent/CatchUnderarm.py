...
class ShadowHandCatchUnderarmGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_pos, self.goal_pos, self.object_linvel, self.fingertip_pos)
...
@torch.jit.script
def compute_reward(object_pos: torch.Tensor, goal_pos: torch.Tensor, object_linvel: torch.Tensor, fingertip_pos: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    distance_to_goal = torch.norm(object_pos - goal_pos, dim=-1)
    vertical_height = object_pos[:, 2]
    velocity_magnitude = torch.norm(object_linvel, dim=-1)

    ideal_throw_height = 1.5
    successful_throw_threshold = 0.4
    distance_bonus = (1.0 - (distance_to_goal / successful_throw_threshold)).clamp(0, 1) * 5.0
    vertical_speed_reward = (velocity_magnitude * 1.5).clamp(0, 2.5)
    height_reward = ((vertical_height / ideal_throw_height).clamp(0, 1) * 2.5)

    goal_proximity_bonus = (distance_to_goal <= successful_throw_threshold).float() * (1.0 - (distance_to_goal / successful_throw_threshold)) * 4.0

    low_throw_penalty = (vertical_height < ideal_throw_height * 0.6).float() * -1.0
    touching_penalty = -torch.clip(torch.norm(fingertip_pos - object_pos.unsqueeze(1), dim=2), max=0.15).sum(dim=1)

    total_reward = distance_bonus + vertical_speed_reward + height_reward + goal_proximity_bonus - low_throw_penalty + touching_penalty

    return total_reward, {
        'distance_bonus': distance_bonus,
        'vertical_speed_reward': vertical_speed_reward,
        'height_reward': height_reward,
        'goal_proximity_bonus': goal_proximity_bonus,
        'low_throw_penalty': low_throw_penalty,
        'touching_penalty': touching_penalty
    }
