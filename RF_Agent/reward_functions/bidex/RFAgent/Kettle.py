...
class ShadowHandKettleGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_pos, self.goal_pos, self.kettle_spout_pos, self.bucket_handle_pos, self.kettle_handle_pos)
...
@torch.jit.script
def compute_reward(object_pos: torch.Tensor, goal_pos: torch.Tensor, kettle_spout_pos: torch.Tensor, bucket_handle_pos: torch.Tensor, kettle_handle_pos: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    distance_to_goal = torch.norm(kettle_spout_pos - bucket_handle_pos, dim=1)
    reward_reach_goal = -distance_to_goal * 2.0  # Stronger incentive for proximity to goal
    
    # Penalty for maintaining kettle stability (increased emphasis on handle position deviation)
    stability_penalty = torch.norm(kettle_handle_pos - object_pos, dim=1)
    reward_stability = -stability_penalty * 1.0  # Stronger penalty for stability
    
    # Bonus for successfully pouring
    tolerance = 0.05  # Tighter tolerance for success
    bonus_goal_reached = (distance_to_goal < tolerance).float() * 2.0  # Increased reward for successfully pouring
    
    # Total reward combines reaching the goal, stability, and pour success
    total_reward = reward_reach_goal + reward_stability + bonus_goal_reached

    # Collecting individual rewards
    reward_components = {
        'reach_goal': reward_reach_goal,
        'stability': reward_stability,
        'pour_success': bonus_goal_reached
    }
