...
class ShadowHandGraspAndPlaceGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_pos, self.goal_pos, self.block_left_handle_pos, self.block_right_handle_pos)
...
@torch.jit.script
def compute_reward(object_pos: torch.Tensor, goal_pos: torch.Tensor, block_left_handle_pos: torch.Tensor, block_right_handle_pos: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # {The reward function incorporates a balanced approach to incentivize reaching the goal, maintaining grasp stability, and successful placement with a stronger emphasis on grasping throughout the task.}
    
    goal_tolerance = 0.05  # Tighter tolerance for reaching the goal
    distance_to_goal = torch.norm(object_pos - goal_pos, dim=1)
    
    # Reach reward with an increased multiplier for being close to the goal
    reach_reward = (distance_to_goal < goal_tolerance).float() * 3.0  # Strong incentive when close to goal
    
    # Assessing grasp based on hand positions relative to the object with smaller grasp distance threshold
    grasp_distance_left = torch.norm(object_pos - block_left_handle_pos, dim=1)
    grasp_distance_right = torch.norm(object_pos - block_right_handle_pos, dim=1)
    grasped = ((grasp_distance_left < 0.03) & (grasp_distance_right < 0.03)).float()
    
    # Stability bonus increased for closer hand proximity while grasping
    grasp_stability_bonus = torch.exp(-grasp_distance_left * 5.0) * torch.exp(-grasp_distance_right * 5.0)  # Higher penalty for distance gives a stronger stability reward
    
    # Encouragement for completing the task with an additional bonus for successful grasp
    place_reward = grasped * (reach_reward * 12.0)  # Increased reward for successful grasping
    total_reward = reach_reward + grasp_stability_bonus + place_reward
    
    return total_reward, {
        'reach_reward': reach_reward,
        'grasp_stability_bonus': grasp_stability_bonus,
