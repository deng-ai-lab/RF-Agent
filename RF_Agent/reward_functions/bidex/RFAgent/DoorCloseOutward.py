...
@torch.jit.script
def compute_reward(object_pos: torch.Tensor, door_left_handle_pos: torch.Tensor, door_right_handle_pos: torch.Tensor, 
                   left_hand_pos: torch.Tensor, right_hand_pos: torch.Tensor, goal_pos: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # {This reward function introduces a proximity reward with dynamic scaling, a combined success reward for interactions with 
    # door handles and the goal, a velocity bonus for efficient movement, and an explorative reward for encouraging diverse actions. }

    # Constants
    handle_tolerance = 0.05
    goal_tolerance = 0.1
    proximity_bonus_weight = 5.0
    success_bonus = 1.0
    velocity_bonus_weight = 0.1
    explorative_bonus_weight = 0.02

    # Calculate distances
    distance_to_left_handle = torch.norm(object_pos - door_left_handle_pos, dim=1)
    distance_to_right_handle = torch.norm(object_pos - door_right_handle_pos, dim=1)
    distance_to_goal = torch.norm(object_pos - goal_pos, dim=1)

    # Calculate proximity rewards considering dynamic scaling
    handle_reward = torch.sigmoid(proximity_bonus_weight * (handle_tolerance - torch.minimum(distance_to_left_handle, distance_to_right_handle)))
    goal_reward = torch.sigmoid(proximity_bonus_weight * (goal_tolerance - distance_to_goal))

    # Check if within the reach thresholds
    handle_reached = (distance_to_left_handle < handle_tolerance).float() + (distance_to_right_handle < handle_tolerance).float()
    goal_reached = (distance_to_goal < goal_tolerance).float()

    # Bonus for successful interactions
    success_reward = success_bonus * (handle_reached + goal_reached)

    # Velocity bonus for action efficiency
    combined_hand_pos = torch.cat((left_hand_pos, right_hand_pos), dim=1)
    hand_velocity = torch.norm(combined_hand_pos, dim=1)
    velocity_bonus = velocity_bonus_weight * hand_velocity

    # Explorative reward for diverse actions
    explorative_reward = explorative_bonus_weight * torch.rand(object_pos.size(0), device=object_pos.device)

    # Totaling the overall reward
    total_reward = handle_reward + goal_reward + success_reward + velocity_bonus + explorative_reward
    ...
    reward_components = {
        ...
        "explorative_reward": explorative_reward,
    }
