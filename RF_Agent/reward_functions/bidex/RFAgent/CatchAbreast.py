...
@torch.jit.script
def compute_reward(object_pos: torch.Tensor, goal_pos: torch.Tensor, left_hand_pos: torch.Tensor, right_hand_pos: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    ...
    success_bonus = torch.where(distance_to_goal < 0.03, torch.tensor(1.0, device=object_pos.device), torch.tensor(0.0, device=object_pos.device))

    # Hand balance reward incentivizing even distribution of effort between hands
    hand_balance_reward = -0.5 * torch.abs(distance_to_left_hand - distance_to_right_hand) * torch.clamp(1.0 - (distance_to_left_hand + distance_to_right_hand) / 1.5, min=0, max=1)
    ...
    proximity_reward = torch.clamp(1.0 - (combined_hand_distance / 0.75), min=0, max=1)
    ...
    smooth_movement_bonus = torch.clamp(1.0 - (hand_distance_between_hands / 0.1), min=0, max=1)

    # Total reward calculation
    total_reward = (distance_reward + success_bonus + hand_balance_reward + proximity_reward + smooth_movement_bonus) / 2.0
