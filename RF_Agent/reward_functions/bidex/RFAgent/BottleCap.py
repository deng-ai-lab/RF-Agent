...
class ShadowHandBottleCapGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_pos, self.bottle_pos, self.bottle_cap_pos, self.left_hand_pos, self.right_hand_pos, self.goal_pos, self.bottle_cap_up)
...
@torch.jit.script
def compute_reward(object_pos: torch.Tensor, bottle_pos: torch.Tensor, bottle_cap_pos: torch.Tensor,
                   left_hand_pos: torch.Tensor, right_hand_pos: torch.Tensor, goal_pos: torch.Tensor,
                   bottle_cap_up: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    
    # Define parameters
    goal_tolerance = 0.1
    stability_threshold = 0.15
    proximity_threshold = 0.1
    goal_reward_value = 3.0
    coordination_penalty_weight = 2.0
    
    # Calculate distance rewards and penalties
    bottle_cap_distance = torch.norm(bottle_cap_pos - bottle_cap_up, dim=-1)
    stability_reward = torch.clamp(1.0 - bottle_cap_distance / stability_threshold, min=0.0)
    ...
    hands_proximity_reward = torch.clamp(1.0 - (left_hand_to_cap_dist + right_hand_to_cap_dist) / (2 * proximity_threshold), min=0.0)

    distance_to_goal = torch.norm(object_pos - goal_pos, dim=-1)
    goal_achieved = (distance_to_goal < goal_tolerance).float()
    goal_reward = goal_achieved * goal_reward_value

    hands_distance = torch.norm(left_hand_pos - right_hand_pos, dim=-1)
    coordination_penalty = torch.clamp(coordination_penalty_weight - hands_distance / proximity_threshold, min=0.0)

    # Total reward calculation
    total_reward = stability_reward * 0.4 + hands_proximity_reward * 0.3 + goal_reward + coordination_penalty * -0.2
