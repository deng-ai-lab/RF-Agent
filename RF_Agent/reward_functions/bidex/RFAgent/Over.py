...
class ShadowHandOverGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_pos, self.goal_pos, self.object_linvel, self.fingertip_another_pos)
...
def compute_reward(object_pos: torch.Tensor, goal_pos: torch.Tensor, 
                   object_linvel: torch.Tensor, fingertip_another_pos: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    goal_threshold: float = 0.05
    height_threshold: float = 0.3
    upward_velocity_bonus: float = 2.5
    contact_penalty_threshold: float = 0.1
    
    # Calculate distance to the goal
    distance_to_goal: torch.Tensor = torch.norm(object_pos - goal_pos, dim=-1)
    
    # Initialize reward
    reward: torch.Tensor = torch.zeros(object_pos.shape[0], device=object_pos.device)
    individual_rewards: Dict[str, torch.Tensor] = {}

    # Strong bonus for reaching near the goal position
    goal_reached: torch.Tensor = distance_to_goal < goal_threshold
    reward += 7.0 * goal_reached.float()
    individual_rewards["goal_reached"] = 7.0 * goal_reached.float()

    # Height reward to ensure the object is thrown sufficiently high
    height_reward: torch.Tensor = (object_pos[:, 2] > height_threshold).float() * 5.0
    reward += height_reward
    individual_rewards["height_reward"] = height_reward

    # Upward velocity reward
    upward_velocity_reward: torch.Tensor = torch.clamp(object_linvel[:, 2], min=0) * upward_velocity_bonus
    reward += upward_velocity_reward
    individual_rewards["upward_velocity_reward"] = upward_velocity_reward

    # Alignment bonus based on throw direction towards the goal
    throw_direction: torch.Tensor = object_linvel / (torch.norm(object_linvel, dim=1, keepdim=True) + 1e-10)
    goal_direction: torch.Tensor = (goal_pos - object_pos) / (torch.norm(goal_pos - object_pos, dim=1, keepdim=True) + 1e-10)
    alignment_bonus: torch.Tensor = (throw_direction * goal_direction).sum(dim=1) * 2.5
    reward += alignment_bonus
    individual_rewards["alignment_bonus"] = alignment_bonus

    # Penalty for contact with the receiving hand
    contact_penalty_dist: torch.Tensor = torch.norm(fingertip_another_pos - object_pos.unsqueeze(1), dim=-1)
    contact_penalty: torch.Tensor = -torch.clamp(contact_penalty_threshold - contact_penalty_dist, min=0).sum(dim=1) * 4.0
    reward += contact_penalty
    individual_rewards["contact_penalty"] = contact_penalty

    # Negative reward for distance to encourage progress towards the goal
    progress_reward: torch.Tensor = -0.5 * distance_to_goal
    reward += progress_reward
    individual_rewards["progress_reward"] = progress_reward
