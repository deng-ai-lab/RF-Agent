...
class ShadowHandSwingCupGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_pos, self.goal_pos, self.cup_left_handle_pos, self.cup_right_handle_pos, self.left_hand_pos, self.right_hand_pos, self.object_angvel, self.object_linvel)
...
def compute_reward(object_pos: torch.Tensor, goal_pos: torch.Tensor, 
                   cup_left_handle_pos: torch.Tensor, cup_right_handle_pos: torch.Tensor,
                   left_hand_pos: torch.Tensor, right_hand_pos: torch.Tensor,
                   object_angvel: torch.Tensor, object_linvel: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Constants
    goal_tolerance = 0.02  # Tolerance for successfully reaching the goal
    goal_reached_bonus = 10.0  # Bonus for achieving the goal
    temp_distance = 0.5  # Temperature for normalizing distance
    temp_handle_proximity = 0.2  # Temperature for handle proximity
    temp_stability = 0.15  # Temperature for stability based on angular velocity
    temp_motion = 0.25  # Temperature for linear motion stability

    # Calculate distance to the goal
    distance_to_goal = torch.norm(object_pos - goal_pos, dim=-1)
    normalized_distance = torch.exp(-distance_to_goal / temp_distance)

    # Check if the cup is at the goal position within tolerance
    goal_reached = (distance_to_goal < goal_tolerance).float()
    goal_reward = goal_reached * goal_reached_bonus

    # Calculate handles proximity rewards
    left_handle_distance = torch.norm(cup_left_handle_pos - left_hand_pos, dim=-1)
    right_handle_distance = torch.norm(cup_right_handle_pos - right_hand_pos, dim=-1)
    handle_proximity_reward = torch.exp(- (left_handle_distance + right_handle_distance) / temp_handle_proximity)

    # Calculate stability reward based on angular velocity
    stability_reward = -torch.exp(-torch.abs(object_angvel[:, 2]) / temp_stability)  # Z-axis is important

    # Calculate motion stability reward based on linear velocity
    motion_stability_reward = torch.exp(-torch.norm(object_linvel, dim=-1) / temp_motion)

    # Total reward calculation
    total_reward = normalized_distance + handle_proximity_reward + goal_reward + stability_reward + motion_stability_reward

    # Create reward components dictionary
    reward_components = {
        'normalized_distance': normalized_distance,
        'handle_proximity_reward': handle_proximity_reward,
        'goal_reward': goal_reward,
        'stability_reward': stability_reward,
        'motion_stability_reward': motion_stability_reward
    }
