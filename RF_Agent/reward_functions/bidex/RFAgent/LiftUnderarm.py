...
class ShadowHandLiftUnderarmGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_pos, self.goal_pos, self.left_hand_pos, self.right_hand_pos, self.pot_right_handle_pos, self.pot_left_handle_pos, self.object_linvel)
...
def compute_reward(object_pos: torch.Tensor, goal_pos: torch.Tensor, 
                   ...
                   pot_right_handle_pos: torch.Tensor, pot_left_handle_pos: torch.Tensor, 
                   object_linvel: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    
    # Constants
    grasp_distance_threshold = 0.12
    lift_bonus_threshold = 0.25
    stability_bonus_threshold = 0.05
    temperature_hand = 10.0
    temperature_lift = 5.0
    temperature_stability = 10.0

    # Reward for grasping both handles
    right_grasped = torch.clamp(torch.exp(-torch.norm(pot_right_handle_pos - right_hand_pos, dim=1) / temperature_hand), min=0.0, max=1.0)
    left_grasped = torch.clamp(torch.exp(-torch.norm(pot_left_handle_pos - left_hand_pos, dim=1) / temperature_hand), min=0.0, max=1.0)

    # Lift reward based on proximity to the goal
    lift_distance = torch.norm(object_pos - goal_pos, dim=1)
    lift_bonus = torch.clamp(torch.exp(-lift_distance / temperature_lift), min=0.0, max=1.0)

    # Stability bonus for maintaining proper motion
    vertical_velocity = object_linvel[:, 2]  
    horizontal_velocity = torch.norm(object_linvel[:, :2], dim=1)
    stability_bonus = torch.where((vertical_velocity > stability_bonus_threshold) & 
                                  (horizontal_velocity < stability_bonus_threshold),
    ...
    excess_movement_penalty = torch.clamp(horizontal_velocity - stability_bonus_threshold, min=0.0)
    
    # Total reward combining components
    total_reward = right_grasped + left_grasped + lift_bonus + stability_bonus - excess_movement_penalty
    ...
    reward_components = {
        ...
        'excess_movement_penalty': -excess_movement_penalty
    }
