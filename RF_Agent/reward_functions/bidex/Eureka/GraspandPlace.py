...
class ShadowHandGraspAndPlaceGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_pos, self.goal_pos, self.left_hand_pos, self.right_hand_pos)
...
def compute_reward(object_pos: torch.Tensor, goal_pos: torch.Tensor,
    ...
    goal_distance_temp = 0.05  # Make goal distance more sensitive
    grasped_temp = 0.5  # Adjusted for better sensitivity in grasping reward

    # Compute distance from the object to the goal
    object_to_goal_distance = torch.norm(object_pos - goal_pos, dim=-1)  # Calculate L2 distance
    # Revise goal distance reward calculation
    goal_distance_reward = torch.exp(-object_to_goal_distance / goal_distance_temp)  # Exponential decay
    goal_distance_reward = torch.clamp(goal_distance_reward, min=0.0, max=1.0)  # Clip to [0, 1]

    # Check if the object is in grasp by either hand (considering a threshold)
    left_grasp_success = (torch.norm(left_hand_pos - object_pos, dim=-1) < 0.1)
    right_grasp_success = (torch.norm(right_hand_pos - object_pos, dim=-1) < 0.1)
    grasp_success = left_grasp_success | right_grasp_success
    
    # Gradual reward for grasping based on successes
    grasped_reward = torch.where(grasp_success, torch.tensor(1.0, device=object_pos.device) * (0.2 + 0.8 * (1 - object_to_goal_distance / 0.5)), 
                                 torch.tensor(0.0, device=object_pos.device))  # Gradual increase based on distance to goal

    # Total reward calculation
    total_reward = goal_distance_reward + grasped_reward
    
    # Prepare components for return
    reward_components = {
        'goal_distance_reward': goal_distance_reward,
        'grasped_reward': grasped_reward
    }
