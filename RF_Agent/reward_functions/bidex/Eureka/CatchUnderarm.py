...
class ShadowHandCatchUnderarmGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_pos, self.goal_pos, self.object_linvel)
...
@torch.jit.script
def compute_reward(object_pos: torch.Tensor, goal_pos: torch.Tensor, object_linvel: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Define temperature parameters for reward transformations
    distance_temp = 10.0  # Increased for more sensitivity on distance
    upward_velocity_temp = 3.0  # Increased sensitivity for upward velocity
    task_score_temp = 15.0  # Balanced sensitivity for task scoring
    success_temp = 5.0  # Maintained influence on policy learning

    # Calculate Euclidean distance to the goal
    distance = torch.norm(object_pos - goal_pos, dim=-1) + 1e-5  # Ensure numerical stability
    distance_reward = torch.exp(-distance * distance_temp)  # Exponential decay for distance approach

    # Rewriting Upward Velocity Reward
    upward_velocity = object_linvel[:, 2]  # Focus on upward movement
    upward_velocity_reward = torch.exp(torch.relu(upward_velocity) - 0.5)  # Encouraging upward velocities

    # Enhanced Task Score calculation
    norm_distance = torch.clamp(distance / 0.5, 0, 1)  # Normalizing distance
    task_score = (1.0 - norm_distance) ** 3 * task_score_temp  # Cubic behavior to accentuate positive actions

    # Success Reward determination
    success_reward = (torch.clamp((1.0 - distance / 0.5), min=0) + 
                      torch.clamp((upward_velocity - 0.5), min=0) * 0.1) * success_temp  # Maintain interaction

    # Calculate total reward
    total_reward = distance_reward + upward_velocity_reward + task_score + success_reward
