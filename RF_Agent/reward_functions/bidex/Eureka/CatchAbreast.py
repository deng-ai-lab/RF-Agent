...
def compute_reward(object_pos: torch.Tensor, goal_pos: torch.Tensor,
    ...
    distance_reward = torch.exp(-distance_temp * distance_to_goal)  # Exponential decay for distance
    ...
    velocity_penalty = torch.where(velocity_magnitude < speed_threshold, -1.0 * (speed_threshold - velocity_magnitude), torch.zeros_like(velocity_magnitude))

    # Positive reward for aligned velocity towards the goal direction
    direction_vector = goal_pos - object_pos  # Direction vector from object to goal
    ...
    aligned_velocity = torch.sum(object_linvel * desired_velocity, dim=1)  # Dot product for alignment
    aligned_velocity_reward = torch.clamp(aligned_velocity, min=0.0)  # Only reward positive velocities
    velocity_reward = aligned_velocity_reward + velocity_penalty  # Combine rewards and penalties
