...
class ShadowHandBottleCapGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_pos, self.bottle_cap_pos, self.left_hand_pos, self.right_hand_pos, self.goal_pos)
...
@torch.jit.script
def compute_reward(object_pos: torch.Tensor, bottle_cap_pos: torch.Tensor, left_hand_pos: torch.Tensor, right_hand_pos: torch.Tensor, goal_pos: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Parameters for reward shaping
    cap_opening_success_temp = 5.0
    bottle_holding_reward_temp = 0.3
    task_success_reward_temp = 20.0
    max_distance_to_goal = 0.1  # Typical distance for successful cap opening
    max_episode_length = 300.0  
    
    # Calculate distances
    distance_to_goal = torch.norm(object_pos - goal_pos, dim=-1)
    distance_bottle_to_left_hand = torch.norm(bottle_cap_pos - left_hand_pos, dim=-1)
    distance_bottle_to_right_hand = torch.norm(bottle_cap_pos - right_hand_pos, dim=-1)

    # Improved cap opening reward: Strong positive feedback for proximity to cap with a binary success criteria
    cap_opening_success = ((distance_bottle_to_left_hand < 0.05) & (distance_bottle_to_right_hand < 0.05)).float()  # Successful opening if hands close enough
    cap_opening_reward = cap_opening_success * torch.exp(-torch.clamp(distance_bottle_to_left_hand + distance_bottle_to_right_hand, min=0) / 0.1)

    # Bottle holding reward: Rescale and emphasize holding proximity
    bottle_holding_reward = torch.exp(-distance_to_goal / 2.0) * (
        (distance_bottle_to_left_hand < 0.1).float() +
        (distance_bottle_to_right_hand < 0.1).float()
    )

    # Task success reward: Reward for achieving proximity to goal
    task_success_reward = ((distance_to_goal < 0.05).float() * task_success_reward_temp)

    # Total reward computation
    total_reward = (
        cap_opening_reward +
        (bottle_holding_reward * bottle_holding_reward_temp) +
        (task_success_reward)
    )

    # Individual rewards in dictionary format
    rewards_dict = {
        "cap_opening_reward": cap_opening_reward,
        "bottle_holding_reward": bottle_holding_reward,
