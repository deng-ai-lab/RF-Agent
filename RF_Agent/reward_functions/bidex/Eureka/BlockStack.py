...
class ShadowHandBlockStackGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_pos, self.block_pos, self.goal_pos, self.left_hand_pos, self.right_hand_pos)
...
@torch.jit.script
def compute_reward(object_pos: torch.Tensor, block_pos: torch.Tensor, goal_pos: torch.Tensor, 
    ...
    stacking_reward_temp = 5.0  # Adjusted for stronger incentivization
    proximity_reward_temp = 0.2   # Adjusted for more responsiveness
    
    # Define success parameters for stacking blocks: stacking near the goal
    target_height_accessor = goal_pos[:, 2] + 0.05
    height_condition = (block_pos[:, 2] >= target_height_accessor - 0.05) & (block_pos[:, 2] <= target_height_accessor + 0.05)

    # Calculate stacking reward with a stronger incentive
    stacking_reward = torch.where(height_condition, 
                                  torch.tensor(7.0, device=object_pos.device),  # Boosted reward for stacking
                                  torch.tensor(0.0, device=object_pos.device))

    # Dynamic Proximity Reward: Surpassing penalty for distance
    distance_left_hand = torch.norm(object_pos - left_hand_pos, dim=-1)
    distance_right_hand = torch.norm(object_pos - right_hand_pos, dim=-1)

    # Adding positive rewards for closeness while applying a max penalty for distance
    proximity_reward_left = torch.clamp(torch.exp(-distance_left_hand), max=1.0)  # Reward decays exponentially with distance
    proximity_reward_right = torch.clamp(torch.exp(-distance_right_hand), max=1.0)  # Reward decays exponentially with distance

    # Total proximity score
    hand_proximity_reward = proximity_reward_left + proximity_reward_right

    # Total reward combining the newly structured stacking and proximity rewards
    total_reward = stacking_reward + hand_proximity_reward

    # Compile individual rewards
    rewards = {
        'stacking_reward': stacking_reward,
        'hand_proximity_reward': hand_proximity_reward,
    }
