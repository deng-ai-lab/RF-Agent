...
def compute_reward(object_pos: torch.Tensor, goal_pos: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Temperature parameters for scaling reward transformations
    temperature_distance = 0.1
    temperature_task_score = 0.2

    # Reward Components

    # 1. Enhanced distance reward to encourage reaching the goal
    distance_to_goal = torch.norm(object_pos - goal_pos, dim=-1)
    max_distance = 1.0  # Assuming the goal and position space is normalized to 1.0
    distance_reward = torch.clip(1.0 - (distance_to_goal / max_distance), 0.0, 1.0)  # Normalize to 1.0 as max
    distance_reward_transformed = torch.exp(distance_reward / temperature_distance)

    # 2. Reward for lifting task within the threshold
    lift_height = object_pos[:, 2]  # Assuming lifting is happening along the z-axis
    lift_height_threshold = 0.1  # Height above which the object is considered properly lifted
    task_score = torch.clip(lift_height - lift_height_threshold, 0.0, None)  # Reward for every cm lifted above the threshold
    task_score_normalized = task_score / (1.0)  # Normalize to maintain addressable range
    task_score_transformed = torch.exp(task_score_normalized / temperature_task_score)

    # Total reward aggregation
    total_reward = distance_reward_transformed + task_score_transformed
