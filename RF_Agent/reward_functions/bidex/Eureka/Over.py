...
class ShadowHandOverGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_pos, self.goal_pos, self.fingertip_pos, self.fingertip_another_pos, self.object_linvel)
...
def compute_reward(object_pos: torch.Tensor, goal_pos: torch.Tensor, fingertip_pos: torch.Tensor,
                   fingertip_another_pos: torch.Tensor, object_linvel: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    
    # Constants for rewards
    proximity_threshold = 0.5  # Proximity distance threshold
    max_task_score = 1.0        # Maximum achievable task score

    # Temperature parameters for each reward component
    proximity_temp = 10.0  
    linear_velocity_temp = 6.0    
    task_score_temp = 1.5 
    ...
    proximity_reward = torch.exp(-proximity_temp * (distance_to_goal / proximity_threshold))

    # Linear velocity reward: Encouragement for keeping a stable velocity
    velocity_magnitude = torch.linalg.norm(object_linvel, dim=-1)
    linear_velocity_reward = torch.exp(-linear_velocity_temp * (velocity_magnitude - 0.5).abs())

    # Task score: Reward based on the efficacy and timely passing of the object towards the goal
    success_condition = torch.clamp(1.0 - (distance_to_goal / proximity_threshold), min=0.0, max=1.0)
    task_score_reward = (success_condition * task_score_temp) + (0.1 * velocity_magnitude)  # Adding a small bonus for velocity

    # Combine the rewards into a total reward
    total_reward = proximity_reward + linear_velocity_reward + task_score_reward

    # Create a dictionary for each reward component
    reward_components = {
