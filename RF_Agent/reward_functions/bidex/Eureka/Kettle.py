...
class ShadowHandKettleGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_pos, self.goal_pos, self.kettle_handle_pos, self.bucket_handle_pos, self.fingertip_pos)
...
@torch.jit.script
def compute_reward(object_pos: torch.Tensor, goal_pos: torch.Tensor, kettle_handle_pos: torch.Tensor, bucket_handle_pos: torch.Tensor, fingertip_pos: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Constants
    proximity_temp = 1.0  # Temperature for proximity reward
    pouring_success_temp = 0.5  # Temperature for pouring success reward
    max_hand_distance = 1.0  # Max distance for hands from targets
    proximity_effect_range = 0.3  # Sensitive proximity range for increased rewards
    pour_angle_threshold = 0.9  # Angle threshold to increase pouring success

    # Distances between hands and kettle/bucket
    left_hand_distance = torch.norm(fingertip_pos[:, 0] - kettle_handle_pos, dim=1)  # Distance to kettle
    right_hand_distance = torch.norm(fingertip_pos[:, 1] - bucket_handle_pos, dim=1)  # Distance to bucket
    
    # Hands proximity reward: The closer to the maximum proximity, the higher the reward, and adding a nonlinear scaling
    hands_proximity_reward = (max_hand_distance - left_hand_distance) + (max_hand_distance - right_hand_distance)
    hands_proximity_reward = torch.exp(torch.clamp(hands_proximity_reward, min=0.0) / proximity_temp)
    hands_proximity_reward = torch.clamp(hands_proximity_reward, max=10.0)  # Re-scaling for consistent outputs
    
    # Normalize distances for pouring success calculation
    kettle_vector = kettle_handle_pos - object_pos  # From object to kettle handle
    bucket_vector = bucket_handle_pos - object_pos  # From object to bucket handle
    
    # Normalize unit vectors
    kettle_length = torch.norm(kettle_vector, dim=1).unsqueeze(1) + 1e-6  # Avoid division by zero
    bucket_length = torch.norm(bucket_vector, dim=1).unsqueeze(1) + 1e-6
    
    kettle_unit_vector = kettle_vector / kettle_length
    bucket_unit_vector = bucket_vector / bucket_length

    # Compute the angle for pouring and derive reward
    cos_theta = torch.clamp(torch.sum(kettle_unit_vector * bucket_unit_vector, dim=1), -1.0, 1.0)
    pouring_success_reward = (cos_theta > pour_angle_threshold).float() * 2.0 + (0.5 * (1.0 - cos_theta))  # Stronger reward for angles closer to threshold
    pouring_success_reward = torch.exp(pouring_success_reward / pouring_success_temp)  # Nonlinear scaling

    # Total reward calculation
    total_reward = hands_proximity_reward + pouring_success_reward

    # Reward components
    reward_components = {
        "hands_proximity_reward": hands_proximity_reward,
        "pouring_success_reward": pouring_success_reward,
    }
