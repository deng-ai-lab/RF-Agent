...
class ShadowHandDoorCloseOutwardGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_pos, self.goal_pos, self.door_left_handle_pos, self.door_right_handle_pos, self.left_hand_pos, self.right_hand_pos)
...
def compute_reward(object_pos: torch.Tensor, goal_pos: torch.Tensor, 
                   door_left_handle_pos: torch.Tensor, door_right_handle_pos: torch.Tensor, 
                   left_hand_pos: torch.Tensor, right_hand_pos: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    
    # Define temperature variables for the reward components
    left_handle_temp = 0.1
    right_handle_temp = 0.1  
    proximity_task_temp = 0.2  
    task_score_temp = 0.5  

    #.Compute distances to door handles
    left_handle_distance = torch.norm(left_hand_pos - door_left_handle_pos, dim=1)
    right_handle_distance = torch.norm(right_hand_pos - door_right_handle_pos, dim=1)

    # New rewards for handles using proximity based scoring
    handle_threshold = 0.5
    reward_left_handle = torch.where(left_handle_distance < handle_threshold,
                                      torch.exp(-left_handle_temp * left_handle_distance),  
                                      torch.tensor(0.01, device=object_pos.device))
    reward_right_handle = torch.where(right_handle_distance < handle_threshold,
                                       torch.exp(-right_handle_temp * right_handle_distance),  
                                       torch.tensor(0.01, device=object_pos.device))

    # Improved proximity scoring that encourages closer goal distance
    goal_distance = torch.norm(object_pos - goal_pos, dim=1)
    success_threshold = 0.2 
    proximity_score = torch.where(goal_distance < success_threshold,
                                   torch.tensor(1.0, device=object_pos.device),
                                   torch.exp(-proximity_task_temp * (goal_distance - success_threshold)))

    # Task score for achieving proximity with added scale for higher success rate based on distance
    task_score = torch.where(goal_distance < 0.5,
                              1.0 - (goal_distance / 0.5),  # More reward as it gets closer
                              torch.tensor(0.0, device=object_pos.device))

    # Normalize the task score with temperature
    task_score_normalized = torch.exp(-task_score_temp * (1.0 - task_score))

    # Combine all rewards into the total reward
    total_reward = reward_left_handle + reward_right_handle + proximity_score + task_score_normalized
    
    # Individual reward components dictionary
    reward_components = {
        "reward_left_handle": reward_left_handle,
        "reward_right_handle": reward_right_handle,
        "proximity_score": proximity_score,
        "task_score": task_score_normalized,
    }
