...
class ShadowHandSwingCupGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_pos, self.cup_right_handle_pos, self.cup_left_handle_pos, self.left_hand_pos, self.right_hand_pos, self.goal_pos, self.object_rot)
...
def compute_reward(object_pos: torch.Tensor,
    ...
    grasp_temp = 0.02  # Increased temperature to enhance grasp sensitivity
    orientation_temp = 0.1  # Adjusted for more responsive penalties
    progress_temp = 0.05  # For effectively rewarding proximity to the goal

    # Calculate grasp reward based on distance
    grasp_distance_right = torch.norm(right_hand_pos - cup_right_handle_pos, dim=-1)
    grasp_distance_left = torch.norm(left_hand_pos - cup_left_handle_pos, dim=-1)
    
    # Define a threshold for effective grasping
    grasp_threshold = 0.1  # Distance threshold for good grasp
    grasp_reward = torch.where(grasp_distance_right < grasp_threshold, 1.0, 
                                torch.exp(-grasp_distance_right / grasp_temp)) * 0.5 + \
                   torch.where(grasp_distance_left < grasp_threshold, 1.0, 
                                torch.exp(-grasp_distance_left / grasp_temp)) * 0.5

    # Calculate orientation reward using angular alignment
    cup_upward_direction = torch.tensor([0.0, 0.0, 1.0], device=object_pos.device)
    object_upward_direction = object_rot[:, 2:3]  # Assuming upward direction is the z-axis of rotation
    alignment = torch.sum(cup_upward_direction * object_upward_direction, dim=-1).clamp(min=0)  # Ensuring non-negative
    misalignment_penalty = 1 - alignment  # Penalty is proportional to misalignment
    orientation_reward = torch.exp(-misalignment_penalty / orientation_temp)  # Responsive scaling

    # New progress reward based on proximity to goal
    progress_distance = torch.norm(object_pos - goal_pos, dim=-1)
    goal_threshold = 0.5  # Define the distance threshold for rewarding proximity
    progress_reward = torch.where(progress_distance < goal_threshold, 1.0, 
                                   torch.exp(-progress_distance / progress_temp))  # Encourage reaching the goal

    # Combine all rewards into total reward with appropriate weightings
    total_reward = (0.4 * grasp_reward + 0.4 * orientation_reward + 0.2 * progress_reward)
    total_reward = torch.clamp(total_reward, min=0.0, max=1.0)  # Normalize total reward
